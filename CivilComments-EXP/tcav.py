...
class TCAVScore(torchmetrics.Metric):
    ...
    def update(self, grads: torch.Tensor):
        with torch.no_grad():
            ...
            CAV_expanded = self.CAV.unsqueeze(0) 

            cos = F.cosine_similarity(grads, CAV_expanded, dim=-1) 
    ...
    def compute(self):
        return torch.cat(self.all_scores, dim=0)  
...
class TCAV(nn.Module):
    ...
    def generate_TCAVs(self, dset_valid, layer_name, target_index=None, score_signed=True, 
        ...
        for i in trange(n_repeat, leave=False):
            ...
            for batch in concept_dl:
                ...
                layer_grads_ = layer_grads_[0][:, 0, :]  

                tcavs_.update(layer_grads_) 
                random_tcavs_.update(layer_grads_)  
    
            
            tcavs.append(tcavs_.compute().detach().cpu().numpy())  
            random_tcavs.append(random_tcavs_.compute().detach().cpu().numpy()) 

        random_tcavs = np.concatenate(random_tcavs, axis=0)  
        tcavs = np.concatenate(tcavs, axis=0)  
    ...
    def attribute(self, layer_name: str, mode: str, target: Union[int, List[int]] = None,
        ...
        if mode == 'inner_prod':
            attributions = torch.matmul(grads, CAV.T)  
        elif mode == 'cosine_similarity':
            grads_norm = torch.norm(grads, dim=1, keepdim=True)  
            cav_norm = torch.norm(CAV, dim=1, keepdim=True).T  
            attributions = torch.matmul(grads, CAV.T) / (grads_norm * cav_norm + 1e-8)  
        else:
